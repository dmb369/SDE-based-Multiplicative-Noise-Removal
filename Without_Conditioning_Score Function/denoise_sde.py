...
# containing images
# GRAYSCALE-SAFE RGB VERSION (NO RGB SPECKLES)
...
def force_grayscale(x):
    """Force RGB tensor to be grayscale-consistent"""
    return x.mean(dim=1, keepdim=True).repeat(1, 3, 1, 1)


def tied_noise_like(x):
    """Generate noise tied across RGB channels"""
    z = torch.randn(
        x.shape[0],
        1,
        x.shape[2],
...
if __name__ == "__main__":
    ...
    for idx, log_x in tqdm.tqdm(enumerate(loader), total=len(loader)):
        ...
        # Langevin refinement (grayscale-safe)
