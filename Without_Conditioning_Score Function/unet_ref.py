...
class SpatialSelfAttention(nn.Module):
    ...
    This is a lightweight attention to capture long-range dependencies in the bottleneck.
    """
    ...
    def forward(self, x):
        # x: B,C,H,W -> reshape to B,C,N where N=H*W
        b, c, h, w = x.shape
        n = h * w
        x_ = self.norm(x)
        x_ = x_.reshape(b, c, n)

        qkv = self.qkv(x_)  # B, 3C, N
        q, k, v = torch.chunk(qkv, 3, dim=1)

        # split heads: B, num_heads, head_dim, N
        ...
        out = out.permute(0, 1, 3, 2).contiguous()  # B,heads,head_dim,N
        out = out.view(b, c, n)
...
class UNet(nn.Module):
    def __init__(self, *, init_channels, out_channels, channels_multipliers=(1,2,4,8), num_res_blocks,
                 ...
                 input_img_resolution, cond_dim: int = 0, global_attn: bool = False,
                 global_attn_heads: int = 4, multi_scale_global_attn: bool = False):
        
        # so cond_dim: int = 0, the default value, means no conditioning
        super().__init__()
        ...
        # conditional embedding (for gray level, spatial stats, similarity, ...)
        self.cond_dim = cond_dim
        if cond_dim and cond_dim > 0:
            self.cemb = nn.Module()
            self.cemb.dense = nn.ModuleList([
                torch.nn.Linear(cond_dim, self.temb_ch),
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                if curr_res in attn_resolutions:
                    # choose spatial self-attention for multi-scale global attention if enabled
                    if multi_scale_global_attn:
                        attn.append(SpatialSelfAttention(block_in, num_heads=global_attn_heads))
                    else:
        ...
        # optional global spatial self-attention at the bottleneck for long-range deps
        self.mid.global_attn = None
        if global_attn:
            self.mid.global_attn = SpatialSelfAttention(block_in, num_heads=global_attn_heads)
    ...
    def forward(self, x, t, cond=None):
        ...
        # conditional embedding: if provided, embed and add to timestep embedding
        if cond is not None and self.cond_dim and self.cond_dim > 0:
            # expect cond shape (batch, cond_dim)
