        ...
        if smooth_indices is not None and idx in smooth_indices:
            for i in range(num_samples):
                if not deterministic:
                    x_ = one_step_denoising(model, x, t, sigmas, device=device)
                else:
...
if __name__ == "__main__":
    ...
    if args.dataset == "celeba":
        ...
        # simple dataset that loads all images from a folder
    ...
    def _parse_device_arg(dev_str: str):
        """Parse device string and return (device_ids_list, torch.device)

        device_ids_list: list of ints for GPUs (empty for CPU)
        torch.device: primary device to use
        """
        s = dev_str.strip()
        if s == "-1" or s.lower() == "cpu":
            return [], torch.device("cpu")
        # support formats: 'cuda:0', 'cuda:0,1', '0,1', '0'
        if s.startswith("cuda:"):
            s = s.split(":", 1)[1]
        ids = [part.strip() for part in s.split(",") if part.strip() != ""]
        gpu_ids = []
        for part in ids:
            try:
                gpu_ids.append(int(part))
            except Exception:
                # ignore non-int parts
                continue
        if len(gpu_ids) > 0:
            return gpu_ids, torch.device(f"cuda:{gpu_ids[0]}")
        # fallback: try to construct torch.device directly
        try:
            return [], torch.device(dev_str)
        except Exception:
            return [], torch.device("cpu")

    device_ids, parsed_device = _parse_device_arg(args.device)
    # prefer parsed_device only if model already on same type; otherwise keep existing `device`
    if parsed_device.type == "cuda":
        device = parsed_device

    if device_ids and device.type == "cuda" and torch.cuda.device_count() >= 1:
        # only keep ids that exist on the machine
        available = list(range(torch.cuda.device_count()))
        use_ids = [i for i in device_ids if i in available]
        if len(use_ids) > 0:
            print("using multiple GPUs:", use_ids)
            model = torch.nn.DataParallel(model, device_ids=use_ids)
        else:
            print("no matching CUDA devices found for requested ids, using:", device)
    else:
    ...
    for e in np.arange(start_epoch, start_epoch + epochs):
        ...
        if e % save_every == 0 or e == epochs - 1:
            ...
            num_show_images = 4
            save_model(f"models/{exp_name}/model_{e}.pkl", model, True)
