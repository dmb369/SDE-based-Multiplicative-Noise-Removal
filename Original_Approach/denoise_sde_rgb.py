...
# UNCONSTRAINED SETTING (RGB SPECKLE ALLOWED)
# MODEL AND TRAINING REMAIN UNCHANGED
...
def sample_noise_removal(
    ...
    for idx, t_ in tqdm.tqdm(
        ...
        t = torch.ones(size=(x.shape[0], 1), dtype=int).to(device) * t_

        if smooth_indices is not None and idx in smooth_indices:
            x_list = []
            for _ in range(num_samples):
                if not deterministic:
                    x_ = one_step_denoising(model, hmodel, x, t, sigmas, device)
                else:
                    x_ = one_step_denoising_deterministic(model, hmodel, x, t, sigmas, device)
                x_list.append(x_)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Noise removal by diffusion",
        description="Noise removal by diffusion",
    )
    ...
    dmodel.load_state_dict(torch.load(args.model_path, map_location=device))
    dmodel.eval()
    ...
    for idx, batch in tqdm.tqdm(enumerate(loader), total=len(loader)):
        log_x = batch[0].to(device)
        ...
        for j, im in enumerate(log_x_denoised):
            # Convert to grayscale ONLY for saving / metrics
            im_gray = im.mean(dim=0, keepdim=True).repeat(3, 1, 1)
            save_image_2(
                im_gray,
