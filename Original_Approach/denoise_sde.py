...
# containing images
# ONLY CHANGE: FORCE GRAYSCALE (NO OTHER MODIFICATIONS)
...
# --------------------------------------------------
# GRAYSCALE FORCE (single helper, no side effects)
# --------------------------------------------------

def force_grayscale(x):
    if x.shape[1] == 3:
...
def one_step_denoising(
    ...
    # FORCE GRAYSCALE (only addition)
    return force_grayscale(log_x_out).to(device)
...
def sample_noise_removal(
    ...
    # FORCE GRAYSCALE ON INPUT
    x = force_grayscale(x_0)
    hist = [x]
...
if __name__ == "__main__":
    ...
    for idx, log_x in tqdm.tqdm(enumerate(loader), total=len(loader)):
        log_x = force_grayscale(log_x[0].to(device))
