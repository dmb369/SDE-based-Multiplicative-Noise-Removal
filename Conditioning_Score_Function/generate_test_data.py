...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", type=str, default="cpu", help="device to run on (cpu, cuda:0, mps etc)")
    ...
    # we'll create sigmas per-batch when processing in case the last batch is smaller
    ...
    if args.dataset == "celeba":
        ...
        # generic folder dataset: simply load all images found in the folder
    ...
    for idx, log_x in pbar:
        # datasets can return (image, label) tuples for some dataset classes
        if isinstance(log_x, (tuple, list)):
            log_x = log_x[0]

        bs = log_x.shape[0]
        log_x = log_x.to(device)

        # make per-batch sigmas/t sized to this batch (handles final smaller batch)
        t = torch.ones(size=(bs, 1), dtype=torch.int64).to(device) * args.ddpm_target_steps
        sigmas_batch = sigmas.unsqueeze(0).repeat(bs, 1).to(device)

        log_x_in, noise_in, var_in = forward_diffusion_gbm(
            log_x, t, sigmas_batch, device=device
        )
