...
def sample_noise_removal(
    ...
    for idx, t_ in tqdm.tqdm(
        ...
        t = torch.ones(size=(x.shape[0], 1), dtype=int, device=device) * t_
        ...
        if smooth_indices is not None and idx in smooth_indices:
            ...
            for _ in range(num_samples):
                if not deterministic:
                    xs.append(
                        one_step_denoising(
                            model, hmodel, x, t, sigmas, device=device, cond=cond
                        )
                    )
                else:
                    xs.append(
                        one_step_denoising_deterministic(
                            model, hmodel, x, t, sigmas, device=device, cond=cond
                        )
                    )
...
if __name__ == "__main__":
    ...
    transform = data_transform(args.img_size, rgb=not args.no_rgb)
